  ...
  function buildLessonPayload() {
    return {
      ...
      quiz: [
        ...
        { prompt: 'Choose a strategy you can use.', answer: 'Draw, count, compare, or tell a story.' },
  ...
  async function answerQuestion(index: number, correct: boolean) {
    ...
    if (session?.id && UUID_RE.test(session.id)) {
      await supabase.from('question_attempts').insert({
        ...
        ai_feedback: correct ? 'Strong strategy and clear explanation.' : 'Needs another scaffolded example.',
      });
    }
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-6 py-8 lg:grid-cols-[1fr_340px]">
          ...
          <div className="rounded-lg border border-[#e7e8e9] bg-white p-6 shadow-sm">
            <div className="mb-5 flex items-center justify-between">
              <div>
                ...
                <h2 className="mt-1 text-2xl font-semibold text-[#191c1d]">Hi {firstName}, start here.</h2>
              </div>
              <button
            ...
            <div className="grid gap-3">
              {(session?.lesson_payload?.slides ?? []).map((slide, index) => (
                <div key={slide.title} className="rounded-lg border border-[#e7e8e9] bg-[#f8f9fa] p-4">
        ...
        <aside className="space-y-5">
          <div className="rounded-lg border border-[#e7e8e9] bg-white p-6 shadow-sm">
            ...
            <h2 className="mt-5 text-center text-lg font-semibold text-[#191c1d]">Rive avatar ready</h2>
          ...
          <div className="rounded-lg border border-[#e7e8e9] bg-white p-6 shadow-sm">
            ...
            <ol className="mt-4 space-y-3 text-sm text-[#424750]">
              <li className="flex gap-2"><Play className="mt-0.5 h-4 w-4 text-[#003461]" /> Session generated or reused.</li>
              <li className="flex gap-2"><Play className="mt-0.5 h-4 w-4 text-[#003461]" /> Each answer writes mastery and XP.</li>
              <li className="flex gap-2"><Play className="mt-0.5 h-4 w-4 text-[#003461]" /> Finish saves the session snapshot.</li>
              <li className="flex gap-2"><Play className="mt-0.5 h-4 w-4 text-[#003461]" /> Next dashboard load recalculates goals.</li>
            </ol>
          </div>
        </aside>
      </main>
