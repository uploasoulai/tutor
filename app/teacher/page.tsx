...
import {
  Home, Users, BarChart3, Calendar, Settings,
...
export default function TeacherDashboardPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (data.user.user_metadata?.role !== "teacher") { router.replace("/student"); return; }
      setUser(data.user);
      setLoading(false);
    });
  }, []);
  ...
  const alerts = [
    ...
    { student: "David Chen", issue: "Below target in reading", grade: "Grade 3", severity: "medium" },
    { student: "Sofia Park", issue: "Hasn't logged in this week", grade: "Grade 2", severity: "low" },
  ];
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-8">
          ...
          <div className="flex flex-col gap-3 mb-8">
            {alerts.map(a => (
              <div key={a.student} className="bg-white border border-[#e7e8e9] rounded-xl p-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={cn("w-2 h-8 rounded-full",
                    a.severity === "high" ? "bg-red-500" : a.severity === "medium" ? "bg-yellow-500" : "bg-gray-300"
                  )} />
                  <div>
                    <p className="font-semibold text-[#191c1d]">{a.student} <span className="text-sm text-[#727781] font-normal">· {a.grade}</span></p>
                    <p className="text-sm text-[#424750]">{a.issue}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button className="px-4 py-2 text-sm font-medium border border-[#c2c6d1] rounded-lg hover:bg-gray-50 transition-colors">Message</button>
                  <button className="px-4 py-2 text-sm font-medium bg-[#003461] text-white rounded-lg hover:bg-[#002b50] transition-colors">Assign Tutor</button>
                </div>
              </div>
            ))}
          </div>

          <p className="text-sm text-[#727781] text-center">More features coming soon — class analytics, curriculum alignment, bulk interventions.</p>
        </div>
      </main>
    </div>
  );
